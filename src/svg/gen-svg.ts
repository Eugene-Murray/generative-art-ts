export namespace SVG {
  export class GenSVG {
     ...
     */
    addEventListener(type: EventTarget, callback: Function): void {
     ...
     * @returns {object} itself.
     */
    animate(keyframes: [] | {}, options: {}): void {
     ...
     * @returns {object} itself.
     */
    rotate(angle: number, cx: number | null = null, cy: number | null = null) {
      let c = this.getCentre();
      let t = this.createTransform();
      cx = cx === null ? c.x : cx;
      cy = cy === null ? c.y : cy;
     ...
     * @returns {object} itself. ?????
     */
    scale(sx: number, sy: number | null = null): void {
      let c = this.getCentre();
     ...
     * @returns {object} itself. ????
     */
    trackCursor(callback: Function | null = null) {
      this.isMainSVG();

      let point = new DOMPoint();

      this.element.addEventListener("pointermove", (event: { clientX: number; clientY: number; }) => {
     ...
     */
    private addTransform(transform: { type: number; matrix: DOMMatrix; }) {
